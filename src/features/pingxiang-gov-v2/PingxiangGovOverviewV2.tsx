...
export default function PingxiangGovOverviewV2({ state }: { state: PingxiangDataState }) {
  ...
  const chartHref = (base: string, period: string, series?: string) => {
    ...
    navigate(`${base}?${updates.toString()}`)
  }
  ...
  return (
    <div className="pxv2-page-stack">
      ...
      <section className="pxv2-bottom-layout">
        ...
        <Panel title="数据更新与归集说明"><div className="pxv2-notice-list"><div><span className="blue"><Database size={19} /></span><p>{dataSourceText(state)}</p></div><div><span className="violet"><ClipboardList size={19} /></span><p>首页、清单、详情和报告均由同一演示数据模型生成。</p></div><div><span className="green"><SearchCheck size={19} /></span><p>数据缺失时显示“暂无数据”，不会自动用其他数字补齐。</p></div></div><div className="pxv2-update-time">最近有效数据时间：{available ? latestRecordTime(state.data) : '暂无数据'}</div></Panel>
      </section>
