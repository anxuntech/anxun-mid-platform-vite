  ...
  Array.isArray(value)
    ? value.map((item, index) => {
      ...
      return {
        ...
        kind,
        tone: String(record.tone || '') as 'blue' | 'green' | 'orange' | 'violet',
      }
    })
    : []
)
...
const toHazardRecord = (projectId: string) => (item: Record<string, unknown>, index: number): HazardRecord => ({
  ...
  display_id: String(item.display_id || item.id || `YH-${index + 1}`),
  ...
  rectification_content: String(item.rectification_content || ''),
  rectified_at: String(item.rectified_at || ''),
  reviewer: String(item.reviewer || ''),
  review_opinion: String(item.review_opinion || ''),
  ...
  linked_patrol_id: String(item.linked_patrol_id || ''),
  demo_data: false,
})
...
const toPatrolRecord = (projectId: string) => (item: Record<string, unknown>, index: number): PatrolRecord => ({
  ...
  display_id: String(item.display_id || item.id || `XJ-${index + 1}`),
  ...
  items: Array.isArray(item.items) ? item.items as PatrolRecord['items'] : [],
  photos: toEvidenceFiles(item.photos),
  linked_hazard_id: String(item.linked_hazard_id || ''),
  qr_code: String(item.qr_code || ''),
  planned_count: Number(item.planned_count || 0),
...
const toWorkPermitRecord = (projectId: string) => (item: Record<string, unknown>, index: number): WorkPermitRecord => ({
  ...
  display_id: String(item.display_id || item.id || `ZY-${index + 1}`),
  ...
  approvals: Array.isArray(item.approvals) ? item.approvals as WorkPermitRecord['approvals'] : [],
  measures: Array.isArray(item.measures) ? item.measures as WorkPermitRecord['measures'] : [],
...
const toTrainingRecord = (projectId: string) => (item: Record<string, unknown>, index: number): TrainingRecord => ({
  ...
  display_id: String(item.display_id || item.id || `PX-${index + 1}`),
  ...
  participants: Array.isArray(item.participants)
    ...
    : [],
  exam_pass_score: Number(item.exam_pass_score || 70),
  attachments: toEvidenceFiles(item.attachments),
  timeline: toTimeline(item.timeline),
  demo_data: false,
})
